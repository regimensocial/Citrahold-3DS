...
NetworkSystem::NetworkSystem() { network_init(); }

responsePair NetworkSystem::init(std::string serverAddress, std::string token) {
    this->serverAddress = serverAddress;

    if (!valid_certificate()) {
        printf("\nWARNING: The TLS certificate is going to be/is expired "
               "(soon). Please update your build!\nWe will not verify the "
               "certificate, this could be insecure!\n");
    }

    return sendRequest(this->serverAddress + "/areyouawake");
}

std::string NetworkSystem::getBase64StringFromFile(std::string fullFilePath,
                                                   std::string filename) {
...
void NetworkSystem::checkVersion(std::string currentVersion) {
    ...
    responsePair response =
        sendRequest(this->serverAddress + ("/softwareVersion"));
...
bool NetworkSystem::download(UploadTypeEnum type, std::string gameID,
    ...
    responsePair response =
        sendRequest(this->serverAddress + (type == UploadTypeEnum::EXTDATA
                                               ? "/downloadMultiExtdata"
                                               : "/downloadMultiSaves"),
                    &data);
    if (response.first == 200) {
        ...
        for (const auto &element : responseJSON["files"].items()) {
            ...
            std::filesystem::path downloadPath =
                (gamePath / "Citrahold-Download" / filename).string();
            ...
            if (filename.find("citraholdDirectoryDummy") == std::string::npos) {
                ...
                if (!file) {
                    ...
                    std::cout << "[" << itemNumber << "/" << numberOfItems
                              << "] " << "Writing " << filename << "\n";
                    file.write((char *)base64DataDecoded.data(),
                               base64DataDecoded.size());
                    file.close();
                }
            } else {
                std::cout << "[" << itemNumber << "/" << numberOfItems << "] "
...
menuItems NetworkSystem::getGamesMenuItems(UploadTypeEnum type) {
    ...
    responsePair response = sendRequest(
        this->serverAddress +
            (type == UploadTypeEnum::EXTDATA ? "/getExtdata" : "/getSaves"),
        &data);
    if (response.first == 200) {
...
std::string
NetworkSystem::getTokenFromShorthandToken(std::string shorthandToken) {
    nlohmann::json data;

    data["shorthandToken"] = shorthandToken;

    // this will return a full token
    responsePair responseAsString =
        sendRequest(this->serverAddress + "/getToken", &data);
    if (responseAsString.first == 200) {
        nlohmann::json response =
            nlohmann::json::parse(responseAsString.second);
...
std::string NetworkSystem::verifyTokenToSetUserID(std::string fullToken) {
    ...
    responsePair response =
        sendRequest(this->serverAddress + "/getUserID", &data);
    if (response.first == 200) {
...
int NetworkSystem::uploadMultiple(UploadTypeEnum uploadType,
    ...
    responsePair response =
        sendRequest(this->serverAddress + (uploadType == UploadTypeEnum::SAVES
                                               ? "/uploadMultiSaves"
                                               : "/uploadMultiExtdata"),
                    &jsonObject);
...
responsePair NetworkSystem::sendRequest(std::string address,
...
void NetworkSystem::cleanExit() { network_exit(); }
